...
class SubdomainCenter:
    def __init__(self, domain_root, proxy, output_file):
        ...
        self.proxy = proxy
        self.source = "Subdomain Center"
        self.results = Results(self.source)

    # free tier limits 2 requests per minute, and maximum 500 results
    def run(self):
        logging.warning(f"[*] starting Subdomain Center query...")
        headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Safari/537.36"
        }
        url = f"https://api.subdomain.center/?domain={self.domain_root}"
        ...
        try:
            ...
            for subdomain in self.results.data[self.source]["subdomains"]:
                logging.info(f"{Fore.LIGHTGREEN_EX}[+] {subdomain}{Style.RESET_ALL}{Fore.WHITE} [SubdomainCenter]")
